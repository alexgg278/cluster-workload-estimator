"""
1. The goal of this test is to use the simulation built in the test 3 to make it a little bit more meaningful in terms
of node and link usage
2. The second goal is to analyze the results of the simulation using pandas to plot node usage time-series
3. The goal is to predict the memory usage of one node in advance with some time-series technique
4. Finally, we try to reproduce similar results but by adding noise to the memory utilization of the nodes. This can be
easily changed by adding a standard deviation for the distribution of memory usage with mean = bytes
